...
PROMPT_PATH = os.path.join(os.path.dirname(__file__), "..", "..", "..", "prompts", "domain_compsci.txt")


def compsci_agent(state: ProjectState) -> dict:
    with open(PROMPT_PATH, "r") as f:
        system_prompt = f.read()

    # Build wiki context for this agent's target files
    target_files = ["software.md", "overview.md"]
    wiki_context = ""
    ...
    # Merge into current_questions (will be collected by orchestrator)
    existing = state.get("current_questions", [])
    return {
        "current_questions": existing + questions,
    }
